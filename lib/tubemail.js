	...
	_setupNode( node, inboundConnection ) {
		...
		if( ! inboundConnection ) node.on( 'lost', () => {
			...
			let interval = setInterval( () => {
				...
				// If we tried to often, give up
				// TODO: Magic number
				if( retries++ >= 3600 ) {

					// We fully lost that node
					this._nodesLostCnt--;
				...
				this._connect( info.listen, info.id ).then( () => {
					...
					if( e instanceof Node.Error ) {

						// The remote node changed its ID -> Don't connect
						this._nodesLostCnt--;
						if( this._nodesLostCnt === 0 && this._nodesConnectedCnt === 0 ) {
							this._setState( S_WARM );
						}
