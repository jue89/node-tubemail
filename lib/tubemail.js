...
class TubeMail extends events.EventEmitter {
		...
		server.on( 'secureConnection', ( socket ) => {
			...
			newNode( socket, this._localID, this._options.listen ).then( ( node ) => {
				...
				for( let n in this._nodesConnected ) {
					this._nodesConnected[n].newNeigh( {
						id: node.id,
						listen: node.listen
					} );
				}
	...
	_setupNode( node ) {
		...
		node.on( 'shutdown', () => {
			...
			this._nodesConnectedCnt--;
			delete this._nodesConnected[ id ];

			// Emit event
			this.emit( 'neighRemove', id );

			// Set to warm state if no other connections are open
			if( this._nodesConnectedCnt === 0 ) this._setState( S_WARM );
