...
class TubeMail extends events.EventEmitter {
	...
	_bindPort() {
		...
		server.on( 'secureConnection', ( socket ) => {
			...
			// Create new node
			newNode( socket, this._localID, this._options.listen ).then( ( node ) => {

				// Timeout and keepalive
				socket.setKeepAlive( true, 10000 );
				socket.setTimeout( 20000, () => node.shutdown() );

				// Tell everbody about our new friend
				for( let n in this._nodes ) {
					this._nodes[n].newNeigh( {
						id: node.id,
						listen: node.listen
					} );
				}
	...
	_setupNode( node ) {
		...
		node.on( 'newNeigh', ( neighInfo ) => {
			...
			if( this._nodes[neighInfo.id] || this._localID == neighInfo.id ) return;

			// Otherwise say hello to our new neighbor
			this._connect( neighInfo.listen.host, neighInfo.listen.port );
	...
	shutdown() { return this._server.then( (s) => {
		...
		for( let n in this._nodes ) this._nodes[n].shutdown();
	...
	getCert( nodeID ) {
		...
		return this._nodes[ nodeID ].getCert();
