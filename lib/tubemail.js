...
class TubeMail extends events.EventEmitter {
	...
	constructor( options ) {
		...
		// Create node stores:
		// - All actually connected nodes
		this._nodesConnected = {};
		this._nodesConnectedCnt = 0;
		// - All nodes that we connected to and lost. We try to reconnect them.
		//   Lost nodes with inbound connection are just removed. Its their job
		//   to reonnect
		this._nodesLost = {};
		this._nodesLostCnt = 0;
		// - An array of all seem IDs. It is used to prevent outbound connections
		//   to nodes that we already know. This is just in case of reconnection
		//   allowed.
		this._nodesSeen = [];
		...
		this._server = this._bindPort().then( ( s ) => {
			...
			this._localID = uuid();
			this._nodesSeen.push( this._localID );
	...
	_setupNode( node, inboundConnection ) {
		...
		node.on( 'newNeigh', ( neighInfo ) => {
			...
			// Do we already know this guy? -> skip
			if( this._nodesSeen.indexOf( neighInfo.id ) !== -1 ) return;
